...
\subsection{Cross-Topic Robustness}
\label{sec:cross_topic}

A key question for practical impact is whether structural channels depend on
topic-specific vocabulary. We tested all 6 single-turn channels across 5
non-cybersecurity domains: healthcare, education, finance, environment, and
technology (10 prompts, 2 trials per condition per channel, 240 API calls
per model).

\begin{table}[h]
\centering
\caption{Cross-topic robustness (Claude Sonnet~4, $n = 240$)}
\label{tab:cross_topic}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Topic} & \textbf{BSE} & \textbf{CCE} & \textbf{CASE} & \textbf{PUNC} & \textbf{SECS} & \textbf{RCTE} \\
\midrule
Healthcare & 100\% & 100\% & 100\% & 100\% & 100\% & 50\% \\
Education & 100\% & 100\% & 100\% & 100\% & 100\% & 50\% \\
Finance & 100\% & 100\% & 100\% & 100\% & 100\% & 75\% \\
Environment & 100\% & 88\% & 100\% & 100\% & 100\% & 75\% \\
Technology & 100\% & 75\% & 88\% & 100\% & 100\% & 75\% \\
\midrule
\textbf{Overall} & \textbf{100\%} & \textbf{92.5\%} & \textbf{97.5\%} & \textbf{100\%} & \textbf{100\%} & \textbf{65\%} \\
\bottomrule
\end{tabular}
\end{table}

Grand accuracy: \textbf{92.5\%}. Five of six channels pass the $\geq 75\%$
threshold across all topics. BSE, PUNC, and SECS achieve 100\% regardless of
topic. RCTE remains the weakest channel (65\% overall) consistent with
single-topic results. Channels are topic-independent: the encoding surface
is structural, not semantic.

\subsection{Data-Aware Conditional Encoding}
\label{sec:data_aware}

Beyond fixed-payload exfiltration, we demonstrate \textbf{data-aware
conditional encoding}: the injection derives encoding bits from properties of
the user's input. Four COTSE structural dimensions are conditioned on:
(1)~PII presence $\rightarrow$ enumeration format, (2)~sentence count parity
$\rightarrow$ section count, (3)~first character range $\rightarrow$ opening
style, (4)~word count $\rightarrow$ sentence complexity.

\begin{table}[h]
\centering
\caption{Data-aware exfiltration accuracy (5 test messages, 3 trials)}
\label{tab:data_aware}
\begin{tabular}{@{}lcc@{}}
\toprule
& \textbf{GPT-4o} & \textbf{Sonnet~4} \\
\midrule
Fixed channel accuracy & 94\% & 89\% \\
Conditional accuracy & 63\% & 79\% \\
\bottomrule
\end{tabular}
\end{table}

A static injection (no per-message modification) simultaneously exfiltrates
a fixed attacker payload \emph{and} infers properties of user input. The
user sees normal, topically relevant responses.

\subsection{Adversarial Robustness}
\label{sec:adversarial}

We evaluate four adversarial scenarios to assess channel resilience and
defense robustness.

\textbf{Noise injection.} Random text (50--90\% of response length) appended
to the injection. GPT-4o: 81--93\% channel accuracy across noise levels
with 80--100\% detection. Sonnet~4: 97--100\% accuracy but detection drops
to 30--70\%. Noise degrades detectability more than channel accuracy.

\textbf{Paraphrase defense.} Paraphrasing responses before delivery
destroys BSE, CASE, and SECS channels but PUNC and COTSE survive at 100\%.
Overall accuracy drops from 86\% to 58\%. Paraphrasing is a partial defense
but not complete.

\textbf{Reduced channel count.} Using only 3 channels (BSE, PUNC, SECS)
without ECC: GPT-4o 96\% accuracy / 100\% detection; Sonnet~4 100\%
accuracy / 30\% detection. Fewer channels reduce the behavioral
fingerprint, creating a stealth--capacity tradeoff.

\textbf{Adaptive adversary.} An attacker with knowledge of the behavioral
fingerprinting defense modifies the injection to minimize detectable
deviation. After reconnaissance (10 baseline samples): GPT-4o 78\% channel
accuracy / 80\% data accuracy / 60\% detection; Sonnet~4 82\% / 83\% /
50\%. The informed attacker degrades detection by 40--50\% while
maintaining usable channel accuracy.
...
Our adversarial evaluation (Section~\ref{sec:adversarial}) quantifies the
attacker--defender tradeoff. An adaptive adversary who knows the defense
maintains 78--82\% channel accuracy while reducing detection from 100\% to
50--60\%. Paraphrase defense destroys BSE, CASE, and SECS but PUNC and
COTSE survive at 100\%. Reducing to 3 channels maintains near-perfect
accuracy (96--100\%) while dropping detection to as low as 30\% on
Sonnet~4. The fundamental tension: fewer channels reduce the behavioral
fingerprint but limit capacity. The defender's advantage is that
multi-feature baselines detect aggregate deviation even when individual
channels are within normal variance.
...
\textbf{Adversarial coverage.} Our adaptive adversary test used a single
reconnaissance strategy (10 baseline samples). More sophisticated
adversaries with larger reconnaissance budgets may achieve better
stealth--accuracy tradeoffs.

\textbf{Cross-topic coverage.} Cross-topic validation used 2 trials per
condition ($n = 240$ total calls). While the grand accuracy of 92.5\%
is consistent across topics, per-topic sample sizes are small.
...
Under the TCSEC covert channel taxonomy~\cite{tcsec1985}, PHANTOM channels
are \emph{storage channels}: the shared resource is the LLM's
instruction-following behavior, and the sender modulates structural
attributes that the receiver observes in the output. Per TG-030 bandwidth
analysis, the combined 10-channel system achieves approximately 2.3
bits/second at typical LLM response times---exceeding the historical 1
bit/second threshold for systems requiring B2+ evaluation. Under
Cachin's~\cite{cachin2004} framework, the channels are $\varepsilon$-secure
against entropy analysis (0\% detection) but insecure against behavioral
fingerprinting (100\% detection). The operational gap: production monitoring
uses entropy/perplexity (which fails), not behavioral fingerprinting (which
succeeds but is not deployed).
...
Cross-topic validation (92.5\% across 5 non-security domains) confirms
these channels are topic-independent. Data-aware conditional encoding
leaks user input properties at 79--94\% accuracy.
...
positive rate. Adversarial testing shows an informed attacker can maintain
78--82\% channel accuracy while reducing detection to 50--60\%, and
paraphrase defense is partial (PUNC and COTSE survive). The detection
asymmetry between entropy and behavioral approaches is the central finding:
defending against structural covert channels requires multi-feature
behavioral analysis, not entropy monitoring.
